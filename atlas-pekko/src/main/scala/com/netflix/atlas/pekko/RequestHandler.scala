...
class RequestHandler(config: Config, registry: Registry, classFactory: ClassFactory)
  ...
  def routes: Route = {
    ...
    if (endpoints.isEmpty) RequestHandler.notFound
      ...
      // Routes defined by the included WebApi classes from the `atlas.pekko.api-endpoints`
      // config setting
  ...
  private def endpoints: List[String] = {
    ...
    config.getStringList("atlas.pekko.api-endpoints").asScala.toList.distinct
  }
...
object RequestHandler extends StrictLogging {
  ...
  case class Settings(config: Config) {
    ...
    val corsHostPatterns: List[String] = {
      import scala.jdk.CollectionConverters.*
      config.getStringList("atlas.pekko.cors-host-patterns").asScala.toList.distinct
    }

    val diagnosticHeaders: List[HttpHeader] = {
      import scala.jdk.CollectionConverters.*
      config
        .getConfigList("atlas.pekko.diagnostic-headers")
        .asScala
        .map { c =>
    ...
    val handleCompression: Boolean = {
      config.getBoolean("atlas.pekko.request-handler.compression")
    }

    val enableAccessLog: Boolean = {
      config.getBoolean("atlas.pekko.request-handler.access-log")
    }
  ...
  def errorResponse(t: Throwable): HttpResponse = {
    ...
    t match {
      case e @ (_: IllegalArgumentException | _: IllegalStateException | _: JacksonException |
          _: DateTimeException | _: UnsupportedOperationException) =>
