...
class SumValueFunctionSuite extends AnyFunSuite {
  ...
  test("random offset, out of order") {
    ...
    assert(n.update(t(3, 13), 1.0) === List(t(2, 0) -> 2.0))
    assert(n.update(t(3, 11), 1.0) === Nil)
    n.close()
    assert(n.result() === List(t(3, 0) -> 2.0, t(4, 0) -> 2.0))
  }
  ...
  test("frequent updates") {
    ...
    assert(n.update(40, 3.0) === List(10 -> 14.0, 20 -> 2.0))
    n.close()
  ...
  test("multi-node updates") {
    ...
    n.close()
    assert(n.result() === List(0 -> 13.0, 10 -> 23.0))
  }
}
