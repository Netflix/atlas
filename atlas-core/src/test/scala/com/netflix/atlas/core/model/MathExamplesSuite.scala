...
class MathExamplesSuite extends BaseExamplesSuite {
  ...
  test("rewrite group by toString with offsets and cq") {
    ...
    assertEquals(
      expr.toString,
      "name,test,:eq,app,foo,:eq,:and,:dist-avg,1h,:offset,(,cluster,),:by"
    )
  }
