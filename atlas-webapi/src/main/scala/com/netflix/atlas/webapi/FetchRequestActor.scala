...
/**
  ...
  */
class FetchRequestActor(graphCfg: GraphConfig)
  ...
  def receive: Receive = {
    ...
    case Acquired if chunks.nonEmpty && chunk == null =>
      ...
      chunks = chunks.tail
      try {
        dbRef ! DataRequest(graphCfg).copy(context = chunk)
      } catch {
        case e: Exception =>
...
object FetchRequestActor {
  ...
  def createResponse(system: ActorRefFactory, graphCfg: GraphConfig): HttpResponse = {
    val source = Source.actorPublisher(Props(new FetchRequestActor(graphCfg)))
    HttpResponse(
